...
export default function OclcSearchPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(`/api/search-oclc?q=${encodeURIComponent(q)}&suggest=1&searchScope=${encodeURIComponent(searchScope)}`)
  ...
  function buildUrl({
    ...
    return `/search-oclc?${params.toString()}`;
  }
  ...
  function buildApiUrl({
    ...
    return `/api/search-oclc?${params.toString()}`;
  }
